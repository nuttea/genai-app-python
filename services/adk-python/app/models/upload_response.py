...
class FileInfo(BaseModel):
    ...
    gcs_uri: str | None = Field(
        None,
        description="Artifact URI (artifact://<id> for ADK artifacts)",
    )
    file_type: str = Field(..., description="File type category (video, image, document)")
    extracted_text: str | None = Field(
        None,
        description="Extracted text content (for text/markdown files)",
    )
...
class UploadResponse(BaseModel):
    ...
    files: list[FileInfo] | None = Field(
        None,
        description="Multiple uploaded files info (for batch uploads)",
    )
