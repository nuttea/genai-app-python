...
class SceneDescription(BaseModel):
    ...
    text_overlay: Optional[str] = Field(
        default=None, description="Text to overlay on screen"
    )
    b_roll: Optional[str] = Field(default=None, description="B-roll suggestions")
    transition: str = Field(
        default="fade", description="Transition to next scene"
    )
...
class VideoMetadata(BaseModel):
    ...
    orientation: str = Field(
        default="vertical_9_16", description="Video orientation"
    )
    music_suggestion: Optional[str] = Field(
        default=None, description="Suggested background music"
    )
    hashtags: list[str] = Field(default=[], description="Suggested hashtags")
    thumbnail_idea: Optional[str] = Field(
        default=None, description="Thumbnail suggestion"
    )
...
class VideoScript(BaseModel):
    ...
    # Additional info
    product_featured: Optional[str] = Field(
        default=None, description="Datadog product featured"
    )
